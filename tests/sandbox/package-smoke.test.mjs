...
test('package smoke resolves the canonical Linux executable instead of bundled helpers', () => {
...
test('package smoke reports expected and available Linux executables', () => {
  ...
  try {
    ...
    assert.throws(
      ...
      /package_smoke\.executable_missing:.*:expected=codetask:available=chrome-sandbox/
    )
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
