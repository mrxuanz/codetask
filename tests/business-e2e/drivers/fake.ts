...
export class FakeDriver implements AgentDriver {
  ...
  private async runFoundation(
    ...
    await mcp.callTool('report_case_result', {
      ...
      status: 'completed',
      summary:
        'Foundation fake exercised chat + Design draft create/list/get (architecture 03)',
      observations: [{ step: 'design_draft', draftId: draft.id }],
      artifacts: { projectId: project.id, threadId: thread.id, draftId: draft.id }
    })
    push('case.reported')
  }
  ...
  private async runDesignDraftSmoke(
    ...
    await mcp.callTool('report_case_result', {
      caseId: input.caseId,
      status: 'completed',
      summary: 'Design draft create → abilities → execution profile → confirm via /api/drafts',
      observations: [{ step: 'confirm', draftId: draft.id, status: draft.status }],
      artifacts: { projectId: project.id, draftId: draft.id }
    })
    push('case.reported')
  }
