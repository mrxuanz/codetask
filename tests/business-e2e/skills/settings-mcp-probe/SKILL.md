...
`PUT` body `settings` must be AgentMcpSettings:

```json
{
  "roles": {
    "conversation": { "<core>": { "<rootKey>": { "business-e2e-probe": { ... } } } },
    "task": { "<core>": { "<rootKey>": { "business-e2e-probe": { ... } } } },
    "verification": { "<core>": { "<rootKey>": { "business-e2e-probe": { ... } } } }
  }
}
```

Do **not** put `conversation` / `task` / `verification` at the top level — that returns `Unknown field: conversation`.  
Pass `expectedRevision` from the prior `codetask_get_mcp_settings` response.
...
6. Restore prior settings (same `roles` shape + current `expectedRevision`).
