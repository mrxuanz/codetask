...
import { updateJobRow, isStaleExecutionLeaseOwner, executionLeaseOwner } from '../../src/server/legacy-shim'
...
describe('evidence hydrate with large objects', () => {
  ...
  it('evidence exceeding 5MB is truncated', async () => {
    const { MAX_TASK_EVIDENCE_BYTES } = await import('../../src/server/legacy-shim')
    const largeEvidence = {
    ...
    const { truncateEvidence } = await import('../../src/server/legacy-shim')
