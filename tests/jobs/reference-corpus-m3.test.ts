...
test('projectTaskReadGrants mounts attachment parent dir only for assigned ref', () => {
  const attA = '/data/assets/attachments/thread-1/att-a/file.png'
  const attB = '/data/assets/attachments/thread-1/att-b/other.png'
  ...
  assert.equal((grants[0] as { path: string }).path, '/data/assets/attachments/thread-1/att-a')

  const roots = readGrantsToReadRoots(grants)
  assert.deepEqual(roots, ['/data/assets/attachments/thread-1/att-a'])
})
