...
describe('architecture 04 DoD', () => {
  ...
  it('migration 051/052 remap actor ownership and are registered', () => {
    ...
    assert.match(index, /migration051ActorIdRemap/)
    assert.match(index, /migration052ProjectsActorId/)
    assert.match(index, /migration056TightenLegacyThreadSchema/)
    assert.match(index, /migration057LegacyOwnerActorId/)
