...
use codeteam_sandbox_policy::models::AdditionalPermissionProfile;
use codeteam_sandbox_policy::models::FileSystemPermissions;
use codeteam_sandbox_policy::models::NetworkPermissions;
...
fn transform_preserves_unrestricted_file_system_policy_for_restricted_network() {
    ...
    let exec_request = manager
        .transform(SandboxTransformRequest {
            ...
            codeteam_linux_sandbox_exe: None,
...
fn transform_linux_seccomp_request(
    codeteam_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: cwd.as_path(),
            codeteam_linux_sandbox_exe: Some(codeteam_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let codeteam_linux_sandbox_exe = std::path::PathBuf::from("/tmp/codeteam-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&codeteam_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(codeteam_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let codeteam_linux_sandbox_exe = std::path::PathBuf::from("/tmp/codex");
    let exec_request = transform_linux_seccomp_request(&codeteam_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some("codeteam-linux-sandbox".to_string())
    );
}
