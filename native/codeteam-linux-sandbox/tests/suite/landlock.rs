...
fn codeteam_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_codeteam-linux-sandbox"));
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        Ok(output) => output.exit_code != 0 && is_bwrap_unavailable_output(&output),
...
async fn test_timeout() {
    if should_skip_bwrap_tests().await {
        // should_panic tests cannot soft-skip; force the expected panic string.
        panic!("Sandbox(Timeout): skipped because bwrap is unavailable");
    }
...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    std::fs::create_dir_all(&dot_codex).expect("create .codeteam");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".codeteam write should be denied under bubblewrap",
    );
...
async fn sandbox_blocks_codex_symlink_replacement_attack() {
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".codeteam symlink replacement should be denied",
    );
    assert_ne!(codex_output.exit_code, 0);
}
...
"#,
    ...
    let mkdir_codex_output = expect_denied(
        ...
        .await,
        "child .codeteam directory creation should be denied",
    );
    assert_ne!(mkdir_codex_output.exit_code, 0);
    assert!(!subdir.join(".codeteam").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = codeteam_linux_sandbox_exe()
        .parent()
