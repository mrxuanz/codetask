...
//! - sensitive subpaths such as `.git`, `.agents`, and `.codeteam` remain
...
use codeteam_sandbox_policy::permissions::is_protected_metadata_name;
use codeteam_sandbox_policy::protocol::FileSystemAccessMode;
use codeteam_sandbox_policy::protocol::FileSystemPath;
use codeteam_sandbox_policy::protocol::FileSystemSandboxPolicy;
use codeteam_sandbox_policy::protocol::FileSystemSpecialPath;
use codeteam_sandbox_policy::protocol::WritableRoot;
...
pub(crate) struct BwrapOptions {
    ...
    /// Explicit read-only bind overlays applied after filesystem policy mounts.
...
fn create_bwrap_flags_full_filesystem(command: Vec<String>, options: BwrapOptions) -> BwrapArgs {
    ...
    for (source, target) in &options.extra_ro_binds {
...
fn create_filesystem_args(
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.codeteam`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".codeteam")
                {
                    return None;
                }
    ...
    for writable_root in &sorted_writable_roots {
        ...
        for unreadable_root in nested_unreadable_roots {
            if is_protected_metadata_subpath_of_writable_root(
                &unreadable_root,
                root,
                &protected_metadata_names,
            ) {
    ...
    remove_synthetic_mount_paths_from_protected_create_targets(&mut bwrap_args);
    Ok(bwrap_args)
}

fn remove_synthetic_mount_paths_from_protected_create_targets(bwrap_args: &mut BwrapArgs) {
    ...
    let synthetic_paths: HashSet<PathBuf> = bwrap_args
        .synthetic_mount_targets
        .iter()
    ...
    bwrap_args
        .protected_create_targets
        .retain(|target| !synthetic_paths.contains(target.path()));
}
...
fn is_protected_metadata_subpath_of_writable_root(
    path: &Path,
    writable_root: &Path,
    protected_metadata_names: &[String],
) -> bool {
    protected_metadata_names
        .iter()
        .any(|name| path == writable_root.join(name))
}
...
mod tests {
    ...
    use codeteam_sandbox_policy::protocol::FileSystemAccessMode;
    use codeteam_sandbox_policy::protocol::FileSystemPath;
    use codeteam_sandbox_policy::protocol::FileSystemSandboxEntry;
    use codeteam_sandbox_policy::protocol::FileSystemSandboxPolicy;
    use codeteam_sandbox_policy::protocol::FileSystemSpecialPath;
    use codeteam_utils_absolute_path::AbsolutePathBuf;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let logical_codex = logical_home.join(".codeteam");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".codeteam"));
    ...
    #[test]
    fn synthetic_metadata_mounts_are_not_also_protected_create_targets() {
        let temp_dir = TempDir::new().expect("temp dir");
        let repo = temp_dir.path().join("repo");
        let runtime = repo.join("data/runtimes/thread/jobs/job/tasks/task/codex");
        let runtime_tmp = runtime.join("tmp");
        let dot_git = runtime_tmp.join(".git");
        std::fs::create_dir_all(repo.join(".git")).expect("create parent .git");
        std::fs::write(repo.join(".git/HEAD"), "ref: refs/heads/main\n").expect("write HEAD");
        std::fs::create_dir_all(&runtime_tmp).expect("create runtime tmp");

        let runtime_root = AbsolutePathBuf::from_absolute_path(&runtime).expect("absolute runtime");
        let runtime_tmp_root =
            AbsolutePathBuf::from_absolute_path(&runtime_tmp).expect("absolute runtime tmp");
        let dot_git_root = AbsolutePathBuf::from_absolute_path(&dot_git).expect("absolute .git");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: runtime_root },
                access: FileSystemAccessMode::Write,
            },
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: runtime_tmp_root,
                },
                access: FileSystemAccessMode::Write,
            },
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: dot_git_root },
                access: FileSystemAccessMode::None,
            },
        ]);

        let args = create_filesystem_args(&policy, &repo, NO_UNREADABLE_GLOB_SCAN_MAX_DEPTH)
            .expect("filesystem args");

        assert_empty_directory_mounted_read_only(&args.args, &dot_git);
        assert!(
            synthetic_mount_target_paths(&args).contains(&dot_git),
            "missing runtime tmp .git should be a transient read-only mount",
        );
        assert!(
            !protected_create_target_paths(&args).contains(&dot_git),
            "helper-created synthetic .git mount target must not be removed by protected-create monitoring",
        );
    }
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_codex = path_to_string(&temp_dir.path().join(".codeteam"));
