...
const PROXY_SOCKET_DIR_PREFIX: &str = "codeteam-linux-sandbox-proxy-";
...
fn harden_bridge_process() -> io::Result<()> {
    ...
    codeteam_process_hardening::disable_process_dumping()
}
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("codeteam-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("codeteam-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("codeteam-linux-sandbox-proxy-x"),
            None
        );
