...
use codeteam_sandbox_policy::error::Result as CodexResult;
...
use codeteam_sandboxing::landlock::CODETEAM_LINUX_SANDBOX_ARG0;
...
pub fn run_main() -> ! {
    ...
    if !use_legacy_landlock {
        let extra_ro_bind = collect_extra_ro_binds(extra_ro_bind);
...
fn run_bwrap_with_proc_fallback(
    ...
    file_system_sandbox_policy: &FileSystemSandboxPolicy,
    network_sandbox_policy: NetworkSandboxPolicy,
    extra_ro_binds: &[(PathBuf, PathBuf)],
    inner: Vec<String>,
    mount_proc: bool,
    ...
    let options = BwrapOptions {
        mount_proc,
        network_mode,
        extra_ro_binds: extra_ro_binds.to_vec(),
        ..Default::default()
    };
...
fn exit_with_bwrap_build_error(err: codeteam_sandbox_policy::error::CodexErr) -> ! {
...
fn apply_inner_command_argv0_for_launcher(
    ...
    if supports_argv0 {
        argv.splice(
            ...
            [
                "--argv0".to_string(),
                CODETEAM_LINUX_SANDBOX_ARG0.to_string(),
            ],
        );
        return;
    }
...
fn collect_extra_ro_binds(paths: Vec<PathBuf>) -> Vec<(PathBuf, PathBuf)> {
    let mut pairs = Vec::new();
    let mut chunks = paths.chunks_exact(2);
    for chunk in &mut chunks {
        pairs.push((chunk[0].clone(), chunk[1].clone()));
    }
    assert!(
        chunks.remainder().is_empty(),
        "--extra-ro-bind requires source and target pairs"
    );
    pairs
}
...
fn emit_sandbox_attestation_if_requested(backend: &str) {
    ...
    if let Ok(line) = serde_json::to_string(&value) {
        eprintln!("__CODETASK_ATTEST__{line}");
    }
}
