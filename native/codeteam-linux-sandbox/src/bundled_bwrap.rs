...
fn candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("codeteam-resources").join("bwrap"));
    if let Some(package_target_dir) = exe_dir.parent() {
        candidates.push(package_target_dir.join("codeteam-resources").join("bwrap"));
    }
...
mod tests {
    ...
    fn finds_standalone_bundled_bwrap_next_to_exe_resources() {
        ...
        let expected_bwrap = temp_dir.path().join("codeteam-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let expected_bwrap = target_dir.join("codeteam-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);
