...
use crate::host_launcher::resolve_host_launcher;
...
/// SAM account names must be <= 20 characters (NetUserAdd rejects longer names with NERR_BadUsername / 2202).
pub const OFFLINE_USERNAME: &str = "CodeteamSandboxOff";
pub const ONLINE_USERNAME: &str = "CodeteamSandboxOn";
...
fn run_setup_refresh_inner(
    ...
    if let Some(host) = resolve_host_launcher(request.codex_home) {
        log_note(
            &format!(
                "setup refresh: spawning desktop host {} {} (payload_len={})",
                host.host_exe.display(),
                host.setup_script.display(),
                b64.len()
            ),
            Some(&sandbox_dir(request.codex_home)),
        );
        let mut command = Command::new(&host.host_exe);
        let status = crate::host_launcher::configure_electron_node_command(&mut command)
            .arg(&host.setup_script)
            .arg(&b64)
            ...
            .map_err(|e| {
                log_note(
                    &format!(
                        "setup refresh: failed to spawn desktop host {}: {e}",
                        host.host_exe.display()
                    ),
                    Some(&sandbox_dir(request.codex_home)),
                );
        ...
        if !status.success() {
            log_note(
                &format!("setup refresh: desktop host exited with status {status:?}"),
                Some(&sandbox_dir(request.codex_home)),
            );
...
pub(crate) fn effective_write_roots_for_setup(
    ...
    let host_granted = write_roots_override.is_some();
    ...
    let write_roots = if host_granted {
        write_roots
    } else {
        filter_user_profile_root_exclusions(write_roots)
    };
...
struct ElevationPayload {
    ...
    otel: Option<codeteam_otel::StatsigMetricsSettings>,
    real_user: String,
    #[serde(default)]
    refresh_only: bool,
}
...
fn find_setup_exe() -> PathBuf {
    ...
    {
        let candidate = dir.join("codeteam-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `codeteam-resources/` next to `codex.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("codeteam-resources")
            .join("codeteam-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("codeteam-windows-sandbox-setup.exe")
}
...
fn run_setup_host_launcher(
        ...
        let status = crate::host_launcher::configure_electron_node_command(&mut command)
            ...
            .map_err(|err| {
                failure(
                    ...
                    format!(
                        "failed to launch desktop host setup ({}): {err}",
                        host.host_exe.display()
                    ),
                )
            })?;
    ...
    let cmd_exe = crate::host_launcher::system_cmd_exe();
    let params = crate::host_launcher::elevated_cmd_script_parameters(
        &host.host_exe,
        &host.setup_script,
        &[payload_b64],
    );
    let exe_w = crate::winutil::to_wide(&cmd_exe);
    ...
    if ok == 0 || sei.hProcess == 0 {
        ...
        return Err(failure(
            code,
            format!("ShellExecuteExW failed to launch elevated desktop host setup: {last_error}"),
        ));
    }
    unsafe {
...
fn run_setup_exe(
    ...
    if let Some(host) = resolve_host_launcher(codex_home) {
        return run_setup_host_launcher(&host, &payload_b64, needs_elevation, codex_home);
    }
    let exe = find_setup_exe();
    let cleared_report = match clear_setup_error_report(codex_home) {
        Ok(()) => true,
...
pub fn run_elevated_setup(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: codeteam_otel::global_statsig_metrics_settings(),
        refresh_only: false,
    };
...
mod tests {
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_codex = extra_write_root.join(".codeteam");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_codex).expect("create extra .codeteam");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_codex).expect("canonical extra .codeteam"),
                explicit_deny,
            ]
            .into_iter()
