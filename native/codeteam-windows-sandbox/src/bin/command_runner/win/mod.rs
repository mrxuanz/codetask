...
use crate::ExitPayload;
use crate::FramedMessage;
use crate::LocalSid;
use crate::Message;
use crate::OutputPayload;
use crate::OutputStream;
use crate::PipeSpawnHandles;
use crate::ResizePayload;
use crate::SandboxPolicy;
use crate::SpawnReady;
use crate::SpawnRequest;
use crate::StderrMode;
use crate::StdinMode;
use crate::allow_null_device;
use crate::create_readonly_token_with_caps_and_user_from;
use crate::create_workspace_write_token_with_caps_and_user_from;
use crate::decode_bytes;
use crate::encode_bytes;
use crate::get_current_token_for_restriction;
use crate::hide_current_user_profile_dir;
use crate::log_note;
use crate::parse_policy;
use crate::read_frame;
use crate::read_handle_loop;
use crate::spawn_process_with_pipes;
use crate::to_wide;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\CodeteamSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<crate::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = crate::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
...
pub fn main() -> Result<()> {
    run_command_runner(std::env::args().skip(1).map(|s| s.to_string()))
}

/// Run the elevated sandbox IPC runner (invoked from desktop host via NAPI / runner-entry.js).
pub fn run_command_runner<I>(args: I) -> Result<()>
where
