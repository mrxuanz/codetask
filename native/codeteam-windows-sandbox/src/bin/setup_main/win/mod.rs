...
fn apply_read_acls(
    ...
    for root in read_roots {
        ...
        let force_sandbox_group_acl = requires_explicit_sandbox_group_acl(root);
        ...
        if builtin_has && !force_sandbox_group_acl {
            continue;
        }
...
fn requires_explicit_sandbox_group_acl(path: &Path) -> bool {
    let Ok(user_profile) = std::env::var("USERPROFILE") else {
        return false;
    };
    let profile_key = path_key(Path::new(&user_profile));
    let path_key = path_key(path);
    path_key == profile_key
        || path_key.starts_with(&format!("{}/", profile_key.trim_end_matches('/')))
}

fn parent_traverse_roots(read_roots: &[PathBuf]) -> Vec<PathBuf> {
...
#[allow(dead_code)] // embedded in lib for NAPI; standalone bin targets removed
pub fn main() -> Result<()> {
    let ret = real_main();
    if let Err(e) = &ret {
...
fn real_main() -> Result<()> {
    ...
    execute_setup_payload_b64(&args[1])
}

/// Execute one-time elevated setup from a base64 JSON payload (NAPI / setup-entry.js).
pub fn execute_setup_payload_b64(payload_b64: &str) -> Result<()> {
...
fn run_read_acl_only(payload: &Payload, log: &mut File) -> Result<()> {
    ...
    if !payload.read_roots.is_empty() {
        ...
        let traverse_roots = parent_traverse_roots(&payload.read_roots);
        apply_read_acls(
            &traverse_roots,
            &subjects,
            log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    if payload.read_roots.is_empty() {
        ...
        log_line(log, "refresh-only mode: applying read ACLs synchronously")?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.codeteam`, and `.agents`.
        //
...
mod tests {
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "CodeteamSandboxOff",
            "online_username": "CodeteamSandboxOn",
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let codex_home = temp.path().join("codex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".codeteam");
