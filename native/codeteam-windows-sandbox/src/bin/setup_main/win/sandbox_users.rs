...
pub const SANDBOX_USERS_GROUP: &str = "CodeteamSandboxUsers";
...
pub fn ensure_local_user(name: &str, password: &str, log: &mut File) -> Result<()> {
    if name.chars().count() > 20 {
        return Err(anyhow::Error::new(SetupFailure::new(
            ...
            format!("sandbox username {name} exceeds Windows SAM limit of 20 characters"),
        )));
    }
    ...
    unsafe {
        ...
        if status != NERR_Success {
            ...
            if status == NERR_USER_EXISTS {
                super::log_line(
                    log,
                    &format!("local user {name} already exists; updating password"),
                )?;
            } else {
                super::log_line(
                    log,
                    &format!(
                        "NetUserAdd failed for {name} code {status} ({})",
                        netapi_status_hint(status)
                    ),
                )?;
            }
            ...
            if upd != NERR_Success {
                super::log_line(
                    log,
                    &format!(
                        "NetUserSetInfo failed for {name} code {upd} ({})",
                        netapi_status_hint(upd)
                    ),
                )?;
                return Err(anyhow::Error::new(SetupFailure::new(
                    SetupErrorCode::HelperUserCreateOrUpdateFailed,
...
fn netapi_status_hint(code: u32) -> &'static str {
    match code {
        2202 => "NERR_BadUsername (username invalid or >20 chars)",
        2221 => "NERR_UserNotFound",
        2224 => "NERR_UserExists",
        2245 => "NERR_PasswordTooShort",
        2246 => "NERR_InvalidPassword",
        _ => "see NetAPI error codes",
    }
}
