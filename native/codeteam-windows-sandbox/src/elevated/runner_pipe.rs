...
/// Resolves the elevated runner launch target. Prefer the desktop host process
/// (`electron.exe` / `node.exe` + `runner-entry.js`) over standalone helper exes.
pub fn find_runner_launch(codex_home: &Path, log_dir: Option<&Path>) -> (PathBuf, Vec<String>) {
    if let Some(host) = resolve_host_launcher(codex_home) {
        return (
            host.host_exe,
            vec![host.runner_script.to_string_lossy().into_owned()],
        );
    }
    let exe = {
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\codeteam-runner-{nonce:x}");
