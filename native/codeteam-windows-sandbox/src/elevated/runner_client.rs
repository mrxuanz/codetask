use crate::host_launcher::sandbox_runner_env;
...
use crate::runner_pipe::find_runner_launch;
...
fn connect_pipe_with_timeout(
    ...
    let mut connect_thread = Some(
        thread::Builder::new()
            .name(format!("codeteam-runner-connect-{pipe_label}"))
            .spawn(move || {
...
pub(crate) fn spawn_runner_transport(
    ...
    let (runner_host, runner_prefix_args) = find_runner_launch(codex_home, log_dir);
    let runner_cmdline = runner_host
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "node.exe".to_string());
    let mut argv_parts: Vec<String> = runner_prefix_args;
    argv_parts.push(format!("--pipe-in={pipe_in_name}"));
    argv_parts.push(format!("--pipe-out={pipe_out_name}"));
    let runner_full_cmd = std::iter::once(quote_windows_arg(&runner_cmdline))
        .chain(argv_parts.iter().map(|arg| quote_windows_arg(arg)))
        .collect::<Vec<_>>()
        .join(" ");
    ...
    let runner_env = sandbox_runner_env(codex_home, &sandbox_creds.username)
        .context("prepare sandbox runner Electron environment")?;
    let env_block = make_env_block(&runner_env);
    ...
    if let Err(err) = connect_result {
        unsafe {
            ...
            // that child instead of leaking a stray `codeteam-command-runner.exe`.
            if pi.hProcess != 0 {
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray `codeteam-command-runner.exe` behind.
