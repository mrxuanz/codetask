...
/** UI job view: Execution JobDetail with status aliased to state (no legacy remapping). */
export type ExecutionJob = JobDetail & { status: JobState }
...
export interface JobsApi {
  ...
  fetchJob(jobId: string): Promise<ApiResponse<{ job: ExecutionJob }>>
  pause(
    jobId: string,
...
async function refetchExecutionJob(jobId: string): Promise<ApiResponse<{ job: ExecutionJob }>> {
