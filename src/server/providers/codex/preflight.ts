 ...
 *
 * Host-identity prepare is authoritative (`authMaterialPresent` from host
 * `~/.codex` / env keys). Running `login status` on a separately installed CLI
 * validates a different executable than the SDK and makes authentication depend
 * on host toolchain-manager shims.
 */
