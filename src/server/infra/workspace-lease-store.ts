...
export type WorkspaceLeaseOwnerKind =
  | 'conversation'
  | 'conversation-turn'
  | 'planner'
  | 'thread_job'
...
function workspaceLeaseTtlMs(): number {
  return workspaceLeaseTtlSec() * 1000
}
...
function listActiveLeaseRowsTx(client: Database.Database): WorkspaceLeaseOccupant[] {
  const rows = client
    .prepare(
      `SELECT id, canonical_workspace_root AS canonicalPath, owner_type AS ownerKind,
...
export function acquireWorkspaceLease(
  ...
  const now = nowMs()
  const leaseOwner = getAppContext().bootId
  const leaseExpiresAt = now + workspaceLeaseTtlMs()
  ...
  const acquired = runImmediateTransaction(client, () => {
    ...
    if (existing) {
      client
        .prepare(
          `UPDATE workspace_leases
           SET canonical_workspace_root = ?, run_id = ?, lease_owner = ?, lease_expires_at = ?
           WHERE id = ? AND status = 'active'`
        )
        .run(canonicalPath, input.runId ?? null, leaseOwner, leaseExpiresAt, existing.leaseId)
    ...
    client
      .prepare(
        `INSERT INTO workspace_leases (
          id, canonical_workspace_root, owner_type, owner_id, run_id, lease_owner,
  ...
  const result = getDb()
    ...
    .where(
      and(
        ...
        eq(workspaceLeases.leaseOwner, leaseOwner),
...
export function releaseWorkspaceLease(input: string | ReleaseWorkspaceLeaseInput): boolean {
  ...
  const rows = getDb()
    .select({
      ownerKind: workspaceLeases.ownerType,
...
export function releaseWorkspaceLeaseForOwner(
  ...
  const rows = getDb()
    ...
    .where(
      and(
        eq(workspaceLeases.ownerType, ownerKind),
...
export function findWorkspaceLeaseConflictSnapshot(
  ...
  const rows = getDb()
    .select({
      ...
      canonicalPath: workspaceLeases.canonicalWorkspaceRoot,
      ownerKind: workspaceLeases.ownerType,
      ownerId: workspaceLeases.ownerId,
      runId: workspaceLeases.runId
    })
    .from(workspaceLeases)
    .where(and(eq(workspaceLeases.status, 'active'), gt(workspaceLeases.leaseExpiresAt, nowMs())))
    .all()
...
export function reclaimStaleWorkspaceLeasesOnStartup(): number {
  ...
  const staleOwner = client
    .prepare(
      `UPDATE workspace_leases
       SET status = 'released', released_at = ?
       WHERE status = 'active' AND lease_owner != ?`
    )
    .run(now, leaseOwner).changes

  activeLeaseByOwner.clear()
  return expired + staleOwner
}

export function getActiveWorkspaceLeaseForOwner(
  ownerKind: WorkspaceLeaseOwnerKind,
  ownerId: string
): WorkspaceLeaseOccupant | null {
  const rows = getDb()
...
export const resetWorkspaceLeaseMemoryForTests = resetWorkspaceLeaseStateForTests
