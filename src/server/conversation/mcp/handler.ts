...
import { mergeDraftReferences } from '../../legacy-shim'
import {
...
async function updateReferenceCorpusItemTool(
  ...
  const { updateDraftReferenceDescription } = await import('../../legacy-shim')
  const messageId = await resolveDraftMessageIdFromSession(session, args)
...
async function removeReferenceCorpusItemTool(
  ...
  const { deleteDraftReference } = await import('../../legacy-shim')
  const messageId = await resolveDraftMessageIdFromSession(session, args)
