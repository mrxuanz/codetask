...
/** Default stream factory: delegates to the package-owned Cursor ACP streamer. */
export function createCursorStreamFactory(
  ...
  return async function* (input, options) {
    const { streamCursorAcpTurn } = await import('../../streamers/cursor-acp')
...
export class CursorDriver extends DelegatingProviderDriver {
  ...
  override async shutdown(): Promise<void> {
    const { getCursorProviderRuntimeRegistry } = await import('../../cursor-acp/runtime-registry')
