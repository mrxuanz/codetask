...
import { getProviderDescriptor } from './spec/descriptors.ts'
...
export function buildClaudeMcpServers(
  ...
  if (url) {
    merged[CODETASK_MANAGER_MCP_SERVER] = buildHttpMcpServerConfig(url)
  }
  return merged
}
...
export function buildOpencodeMcpServers(
  ...
  if (url) {
    merged[CODETASK_MANAGER_MCP_SERVER] = {
      type: 'remote',
      url,
      enabled: true,
      headers: {
...
export function listMergedMcpServerNames(
  ...
  if (systemMcpUrl) names.push(CODETASK_MANAGER_MCP_SERVER)
  return names
}
