import type { SettingsProviderCode, ProviderRuntimeSetting, ProviderRuntimeSettings } from '@codetask/contracts'
import { SettingsError } from './settings-errors.ts'
import { contentHash, PROVIDER_CODES } from './setting-namespace.ts'
...
function parseExecutable(value: unknown, path: string): ProviderRuntimeSetting['executable'] {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw SettingsError.badRequest('settings.invalid_payload', `${path}.executable must be an object`)
  }
  const record = value as Record<string, unknown>
  for (const key of Object.keys(record)) {
    if (key !== 'mode' && key !== 'path') {
      throw SettingsError.badRequest('settings.invalid_payload', `${path}.executable.${key} is not supported`)
    }
  }
  if (record.mode === 'path') {
    const pathValue = optionalTrimmedString(record.path)
    if (!pathValue) {
      throw SettingsError.badRequest('settings.invalid_payload', `${path}.executable.path is required`)
    }
    return { mode: 'path', path: pathValue }
  }
  if (record.mode !== 'auto') {
    throw SettingsError.badRequest('settings.invalid_payload', `${path}.executable.mode must be auto or path`)
  }
  return { mode: 'auto' }
}

function parseProviderRuntimeSetting(value: unknown, code: SettingsProviderCode): ProviderRuntimeSetting {
  ...
  for (const key of Object.keys(record)) {
    if (!['enabled', 'executable', 'model', 'endpoint', 'approveMcps'].includes(key)) {
      throw SettingsError.badRequest('settings.invalid_payload', `providers.${code}.${key} is not supported`)
    }
  }
  return {
    enabled: typeof record.enabled === 'boolean' ? record.enabled : defaults.enabled,
    executable: record.executable !== undefined ? parseExecutable(record.executable, `providers.${code}`) : defaults.executable,
    model: record.model !== undefined ? optionalTrimmedString(record.model) : defaults.model,
    endpoint: record.endpoint !== undefined ? optionalTrimmedString(record.endpoint) : defaults.endpoint,
...
export function parseProviderRuntimeSettings(value: unknown): ProviderRuntimeSettings {
  ...
  for (const code of Object.keys(providerRecord)) {
    if (!(PROVIDER_CODES as readonly string[]).includes(code)) {
      throw SettingsError.badRequest('settings.provider_unknown', `Unknown provider: ${code}`)
    }
  }
  const providers = { ...defaults.providers }
  for (const code of PROVIDER_CODES) {
    if (providerRecord[code] !== undefined) {
      providers[code] = parseProviderRuntimeSetting(providerRecord[code], code)
    }
  }
  return { providers }
}
...
export function validateProviderRuntimeSettings(
  ...
  for (const code of PROVIDER_CODES) {
    ...
    if (!setting) {
      throw SettingsError.badRequest('settings.invalid_payload', `Missing provider settings for ${code}`)
    }
