...
export function validatePlanReferenceIds(
  ...
  /** When true (default), every frozen reference id must be assigned to at least one task. */
  ...
  if (requireFullCoverage && available.size > 0) {
    const used = collectPlanReferenceIds(plan)
    const missing = [...available].filter((id) => !used.has(id))
    if (missing.length > 0) {
      throw new Error(
        `incomplete reference coverage. Unassigned referenceIds: [${missing.join(', ')}]`
      )
    }
  }
}
