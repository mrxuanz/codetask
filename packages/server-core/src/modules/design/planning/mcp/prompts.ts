import type { DraftSnapshot, DraftReference, AcceptanceCriterion } from '@codetask/contracts'

/** Same production bar string used by conversation / legacy planner prompts. */
...
function formatDraftReferenceIndexLine(ref: DraftReference): string {
  const pathLine = ref.resolvedPath
...
function buildFrozenReferenceSection(references: DraftReference[]): string {
  ...
  const lines = [
    ...
    ...references.map((reference) => {
      ...
      const displayPath = reference.resolvedPath || reference.assetUrl || reference.localPath
      if (displayPath) parts.push(`  path: ${displayPath}`)
...
function formatAcceptanceCriteria(acceptance: AcceptanceCriterion[]): string {
  if (!acceptance.length) return 'None specified'
  return acceptance
    .map(
      (item, index) =>
        `${index + 1}. Given ${item.given}\n   When ${item.when}\n   Then ${item.then}`
    )
    .join('\n')
}
...
export function buildPlannerUserMessage(input: {
  ...
  const referenceIndex =
    references.length > 0
      ? references.map((item) => formatDraftReferenceIndexLine(item)).join('\n')
      : '- No source references available'

  const referenceSection = buildFrozenReferenceSection(references)
  ...
  const body = [
    ...
    '',
    formatAcceptanceCriteria(draft.acceptance),
    '',
    ...
    '## Workspace',
    '',
    draft.workspaceRoot || input.workspacePath || 'Not specified',
    '',
  ...
  const regeneration = input.regenerationInstruction?.trim()
  if (!regeneration) return body
  return [
    body,
    '',
