import { parseCompleteSliceVerification } from './slice-verdict-tool.ts'
import { getSliceVerifierMcpSession } from './slice-session.ts'
...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        summary: { type: 'string' },
        satisfiedSignals: { type: 'array' },
        missingSignals: { type: 'array' },
        questionableClaims: { type: 'array' },
...
function dispatchTool(
  ...
  if (!session) throw new Error(`Slice verifier session "${sessionId}" not found`)

  if (toolName !== 'complete_slice_verification') {
    throw new Error(`Unknown tool: "${toolName}"`)
  }

  const verdict = parseCompleteSliceVerification(argumentsValue)
