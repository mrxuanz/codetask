...
export class QueryJobService {
  ...
  get(actor: Actor, jobId: string) {
    const job = this.jobs.requireById(jobId)
    this.assertOwner(actor, job.actorId)
    return this.jobs.toDetail(
      job,
      this.queue.getPosition(job.id, job.executionGeneration)
    )
  }

  getTree(actor: Actor, jobId: string) {
    const job = this.jobs.requireById(jobId)
    this.assertOwner(actor, job.actorId)
    return this.jobs.getTree(jobId, job.executionGeneration)
  }

  getWork(actor: Actor, jobId: string, workId: string) {
    const job = this.jobs.requireById(jobId)
    this.assertOwner(actor, job.actorId)
    return this.work.requireWork(jobId, workId)
  }

  getEvidence(actor: Actor, jobId: string, workId: string) {
    const job = this.jobs.requireById(jobId)
    this.assertOwner(actor, job.actorId)
  ...
  listVerifications(actor: Actor, jobId: string) {
    const job = this.jobs.requireById(jobId)
    this.assertOwner(actor, job.actorId)
