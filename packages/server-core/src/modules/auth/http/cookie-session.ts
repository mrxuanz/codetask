...
import { hmacParts } from '../infrastructure/scrypt-password-hasher.ts'
...
export function createCsrfToken(authSecret: string): string {
  ...
  const signature = hmacParts(authSecret, 'csrf:', nonce)
...
export function verifyCsrfToken(authSecret: string, token: string | undefined): boolean {
  ...
  const expected = hmacParts(authSecret, 'csrf:', nonce)
