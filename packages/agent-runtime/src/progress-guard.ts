...
import { sandboxTurnDebug } from './debug/sandbox-turn.ts'
import type { TurnActivityKind } from './turn-scope.ts'
import { stalledAfterMsForRole } from './turn-timeouts.ts'
...
export const DEFAULT_TURN_CONFIG: TurnConfig = {
  ...
  longRunningToolCapMs: 30 * 60 * 1000
}
...
export function longRunningToolCapMs(
  configuredMs = DEFAULT_TURN_CONFIG.longRunningToolCapMs
): number {
  return configuredMs
}
...
export class ProgressGuard {
  ...
  constructor(role: ConversationRole, config: Partial<ProgressGuardConfig> = {}) {
    this._role = role
    this._config = {
      progressWindowMs: DEFAULT_TURN_CONFIG.progressWindowMs,
      stalledMs: DEFAULT_TURN_CONFIG.stalledMs,
      longRunningToolCapMs: DEFAULT_TURN_CONFIG.longRunningToolCapMs,
      ...config
    }
  }
