 ...
 *
 * Desktop and standalone entry points provide host-specific configuration so the business
...
export async function startAppServer(
  ...
  if (storage.phase !== 'ready') {
    ...
    const gate = cli.mode === 'server' ? createProcessSetupGateSecret() : null
    ...
    const setupApp = createSetupShell({
      ...
      validateSetupToken: gate ? (token) => validateSetupToken(gate, token) : undefined,
...
export async function stopAppServer(): Promise<void> {
  ...
  const serverClosePromise = server
    ...
    : Promise.resolve()
  try {
    const { stopRetentionJanitor } = await import('../../../src/server/retention/lifecycle')
    stopRetentionJanitor()
    const { stopArtifactExpiryScheduler } =
      await import('../../../src/server/retention/expiry-scheduler')
  ...
  try {
    const { stopAuthJanitor } = await import('../../../src/server/auth/janitor')
    stopAuthJanitor()
  } catch (error) {
  ...
  await import('../../../src/server/sandbox/supervisor-manager').then((module) =>
    module.shutdownSandboxSupervisor()
  )

  // Do not close SQLite while accepted HTTP/SSE requests are still draining.
  await serverClosePromise

  try {
    const { closeDatabaseForTests } = await import('../../../src/server/db')
