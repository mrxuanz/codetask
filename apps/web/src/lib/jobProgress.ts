...
export function resolveTaskCli(
  task: Pick<UnifiedTaskNode, 'abilityCode' | 'providerCode'>,
  abilities: Array<{ abilityCode: string; recommendedCoreCode?: string }> = []
): string {
  if (task.providerCode?.trim()) return coreLabel(task.providerCode.trim())
...
export function buildPlanTree(
  ...
  const tree = buildUnifiedProgressTree({
    ...
    taskProgressItems: job.taskProgress?.tasks as
      | Parameters<typeof buildUnifiedProgressTree>[0]['taskProgressItems']
      | undefined,
...
export function resolveJobListStatusBadge(
  ...
  if (isExecutionDisplayStatus(status)) {
    ...
    const queueLabel =
      status === 'pending' || status === 'queued' ? formatExecutionQueueLabel(t, job?.queue) : null
    return {
