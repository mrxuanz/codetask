...
const activeThread = computed(() => {
  return workspace.threads.value.find((item) => item.id === workspace.activeThreadId.value) ?? null
})
...
const composerProviders = computed(() =>
...
const currentProviderCode = computed(() => {
  ...
  if (fromThread && composerProviders.value.some((core) => core.code === fromThread))
    return fromThread
  const preferred = getPreferredProviderCode()
  if (
    preferred &&
    composerProviders.value.some((core) => core.code === preferred && core.available)
  ) {
    return preferred
  }
  return (
    composerProviders.value.find((core) => core.available)?.code ??
    composerProviders.value[0]?.code ??
    ''
  )
})

const selectedProvider = computed(() =>
  cores.value.find((core) => core.code === currentProviderCode.value)
)
  ...
  <div v-else class="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background">
    ...
    >
      <div class="flex min-w-0 items-center gap-2">
        ...
        <span v-if="providerSwitching" class="text-xs text-muted-foreground">
