...
import type { JobReferenceManifestDto } from '@codetask/contracts'
...
const emit = defineEmits<{
  updateTaskCli: [providerCode: string]
  saveNodeField: [
    payload: {
      nodeRef: string
...
const selectedProviderCode = computed(() => {
  ...
  if (!task) return ''
  if (task.providerCode?.trim()) return task.providerCode.trim()
...
function handleCliChange(event: Event): void {
  ...
  if (!value || value === selectedProviderCode.value) return
