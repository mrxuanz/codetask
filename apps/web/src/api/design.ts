...
}): Promise<ApiSuccess<DesignDraftDto[]>> {
...
export async function getDesignDraft(draftId: string): Promise<ApiSuccess<DesignDraftDto>> {
...
export async function startPlanningSession(
  draftId: string,
  expectedRevision: number
): Promise<ApiSuccess<PlanningSessionDto>> {
  const body = { expectedRevision }
  assertBody(CreatePlanningSessionBodySchema, body, 'planning session')
  return api<PlanningSessionDto>(`/api/drafts/${encodeURIComponent(draftId)}/planning-session`, {
    method: 'POST',
    body: JSON.stringify(body)
  })
}
...
export async function listPlanningSessionsForDraft(
  ...
  return api<PlanningSessionDto[]>(`/api/drafts/${encodeURIComponent(draftId)}/planning-sessions`)
}
...
export async function patchPlanningTreeNode(
  ...
  body: {
    ...
    abilityCode?: string
    /** Canonical provider; serialized as coreCode on the Design wire schema. */
    providerCode?: string
...
): Promise<ApiSuccess<{ session: PlanningSessionDto; jobId: string }>> {
...
export async function listDesignDraftReferences(
  draftId: string
): Promise<ApiSuccess<DraftReference[]>> {
