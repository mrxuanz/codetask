...
export function provideDraftPlanWorkspace(options: {
  ...
  function watchThread(threadId: string): void {
    ...
    threadHubRelease = realtime.watchTopic(conversationTopic(threadId), (envelope) => {
      ...
      if (envelope.type === 'conversation.changed' || envelope.type === 'message.committed') {
        void (async () => {
          const before = new Set(drafts.value.map((d) => d.draftId || d.messageId))
          await loadWorkspace()
          const created = drafts.value.find((d) => !before.has(d.draftId || d.messageId))
          if (created) {
            await selectDraft(created.draftId || created.messageId)
            setStep(1)
          }
        })()
      }
  ...
  const selectedDraft = computed(() => (selectedDraftId.value ? selectedDraftPayload.value : null))
  ...
  function resolveDraftStepForDraft(draft: ThreadDraftSummaryDto): number {
    const payload =
      (draft.draftId || draft.messageId) === selectedDraftId.value
  ...
  async function loadWorkspace(): Promise<void> {
    ...
    try {
      ...
      if (initialId && drafts.value.some((d) => (d.draftId || d.messageId) === initialId)) {
  ...
  async function selectDraft(messageId: string): Promise<void> {
    const threadId = options.threadId.value
    if (!threadId) return
    const draft = drafts.value.find((d) => (d.draftId || d.messageId) === messageId)
    if (!draft) return
    ...
    try {
      ...
      selectedDraftPayload.value =
        selectedDraftPayload.value?.draftId === messageId
          ? selectedDraftPayload.value
