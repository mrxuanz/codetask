...
export type HomeThread = ConversationListItemDto
...
function createConversationInput(): { providerCode?: string } {
  const providerCode = getPreferredProviderCode()
...
export function provideHomeWorkspace(hub: RealtimeGateway): HomeWorkspaceContext {
  ...
  async function addLocalProject(workspaceRoot: string): Promise<HomeProject> {
    ...
    const threadRes = await createConversationItem(project.id, createConversationInput())
  ...
  async function createNewThread(projectId: string): Promise<HomeThread> {
    const res = await createConversationItem(projectId, createConversationInput())
  ...
  async function renameThreadTitle(threadId: string, title: string): Promise<HomeThread> {
    const res = await renameConversationItem(threadId, title)
    const thread = res.data
  ...
  watch(
    ...
    (threadId) => {
      ...
      threadHubRelease = hub.watchTopic(conversationTopic(threadId), (envelope) => {
        ...
        const thread = conversationFromRealtimePayload(realtimePayload(envelope).conversation)
